...
def create():
    ...
    return jsonify({"msg":"Yeiii, prenda creada con exito", "service_id":service.id}),200

@service_bp.route("/get-all", methods=["GET"])
def get_all():
    services = get_services()
    return jsonify({"msg":"Wuuju, listado de prendas se ejecuto con exito", "services":services}),200

@service_bp.route("/update/<int:service_id>", methods=["PUT"])
def update(service_id):
    data = request.json
    service = update_service(service_id, data)
    return jsonify({"msg": "Yeii, prenda actalizada con exito","service_id":service_id}),200
